...
describe('settings', () => {
  ...
  describe('addRepository', () => {
    ...
    it('新しいリポジトリを追加する', () => {
      ...
      const repos = JSON.parse(container.storageClient.getProperty('GITHUB_REPOSITORIES') ?? '[]');
  ...
  describe('プロジェクトグループ管理', () => {
    ...
    describe('getProjects', () => {
      ...
      it('設定されたプロジェクトを取得する', () => {
        container.storageClient.setProperty(
          'PROJECT_GROUPS',
          JSON.stringify([
            {
              name: 'project1',
    ...
    describe('addRepositoryToProject', () => {
      ...
      it('存在しないプロジェクトにはエラーを投げる', () => {
        expect(() => addRepositoryToProject('nonexistent', 'owner', 'repo')).toThrow(
  ...
  describe('formatDiagnosticResult', () => {
    ...
    it('警告がある場合は適切なメッセージを表示', () => {
      const result = {
        items: [{ name: 'Test', status: 'warning' as const, message: '警告です' }],
        hasErrors: false,
        hasWarnings: true,
      };
    ...
    it('全て正常な場合は適切なメッセージを表示', () => {
      const result = {
        items: [{ name: 'Test', status: 'ok' as const, message: '正常です' }],
        hasErrors: false,
        hasWarnings: false,
      };
