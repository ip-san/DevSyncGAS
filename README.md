...
でも、感覚ではなくデータで判断したい。レビュー時間、手戻り、全体的な改善を数字で証明したい。

DevSyncGASは、そのためのツールです。
...
代わりに注目されているのが、DORA指標（DevOps Research and Assessment）。Google Cloudの研究チームが確立した、本番リリースまでの実際の価値提供を測る4つの指標です。
