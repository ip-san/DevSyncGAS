...
| 期間 | デプロイ回数 | デプロイ頻度（回/日） |
|------|-------------|---------------------|
| 2024-01-01〜01-31 | 25 | 0.81 |

**数値の意味:**
- `0.81回/日` = 30日で25回デプロイ = 約1.2日に1回デプロイ

### DORA分類基準（参考）

デプロイ頻度は**数値（回/日）**で記録されます。DORA分類は以下の基準で判定できます：

| レベル | 基準（回/日） | 目安 |
|--------|-------------|------|
| Elite | >= 1.0 | 1日1回以上 |
| High | >= 0.14 | 週1回以上（1/7） |
| Medium | >= 0.03 | 月1回以上（1/30） |
| Low | < 0.03 | 月1回未満 |
...
> **📌 Note:** 以下の例は、[デプロイ記録ワークフロー](DEPLOYMENT_RECORDING.md)を設定**後**の状態です。初期状態では、デプロイ頻度は `yearly` と表示されます。
