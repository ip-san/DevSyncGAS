...
| ファイル | 上限行数 | 現在 | 目的 |
|---------|---------|------|------|
| CLAUDE.md | 80行 | 66行 | プロジェクト概要、最頻出コマンド |
| CLAUDE_COMMANDS.md | 160行 | 117行 | コマンド・GAS関数リファレンス |
| CLAUDE_TASKS.md | 120行 | 112行 | タスク別フロー |
| CLAUDE_NAV.md | 110行 | 103行 | ナビゲーション、索引 |
| CLAUDE_ARCH.md | 200行 | 130行 | アーキテクチャ概要 |
| **合計** | **670行** | **528行** | - |
...
### 追跡すべき指標
- **総行数**: 528行（目標: 670行以下）
- **平均ファイルサイズ**: 106行（目標: 134行以下）
- **最大ファイルサイズ**: 130行（目標: 200行以下）
...
After:  597行（ARCH:199, TASKS:112, NAV:103, COMMANDS:117）
削減:   303行（34%削減）
