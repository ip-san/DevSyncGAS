...
## 📚 30秒でわかる：このツールで計測する指標

### DORA指標とは？

**DORA（DevOps Research and Assessment）指標**は、Googleが研究したソフトウェア開発チームのパフォーマンスを測る指標です。

**なぜ重要？**
- 🚀 **デプロイ頻度**：バグ修正や新機能が早くユーザーに届く
  - 例：月1回 → 毎日デプロイに改善すると、緊急対応が迅速に
- ⚡ **リードタイム**：コード変更から本番リリースまでの時間
  - 例：72時間 → 2時間に短縮すると、フィードバックサイクルが加速
- 🔧 **変更障害率**：デプロイの何%で問題が発生するか
  - 例：5%なら安全、30%以上は品質改善が必要
- ⏱️ **MTTR**：障害発生から復旧までの時間
  - 例：8時間 → 30分に短縮すると、サービス停止の影響を最小化

**DevSyncGASは、これらの指標を自動的にGitHubから収集してスプレッドシートに表示します。**
...
#### 1.2 clasp（Google Apps Scriptコマンドライン）のインストール

**claspとは？**
Google Apps Scriptをコマンドラインから操作するツールです。

Google Apps Scriptは、Googleが提供する「スプレッドシートを自動化するプログラミング」の仕組みです。DevSyncGASは、このGoogle Apps Scriptで動作するため、claspが必要です。

**具体的には：**
- DevSyncGASのコードを開発パソコンからGoogle Apps Scriptにアップロード
- GASエディタを開く
- スクリプトを実行する
...
スマートフォンで、アプリがカメラやGPSにアクセスするときに「このアプリにアクセスを許可しますか？」と聞かれることがありますよね。

GitHubトークンは、それと同じ仕組みです。

DevSyncGAS（自動化ツール）が：
- 「このリポジトリのPR一覧を見てもいい？」
- 「デプロイメント情報を読んでもいい？」

と言ったときに、GitHubに確認してもらうための証明書です。

**セキュリティ面での安心：**
- トークンは「見るだけ（読み取り専用）」の権限しかないので、コードを変更されたり、削除されることはありません。
...
DevSyncGASが「どのスプレッドシートに指標を書き込むのか」を指定する必要があります。スプレッドシートIDは「スプレッドシートの住所」のようなものです。

例えば：
- スプレッドシートA（営業チーム用）
- スプレッドシートB（開発チーム用）

があった場合、どちらに書くのか？を指定するために必要です。
